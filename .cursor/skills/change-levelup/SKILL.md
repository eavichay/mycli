---
name: change-levelup
description: Contribute lessons from a completed change to team-ai-directives
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: preset:agentic-change
disable-model-invocation: false
---

# Speckit Change Levelup Skill
...
- **1–3 fail** → Flag as "low-signal" with warnings per criterion. Continue (not a hard stop — change mode)
...
git checkout -b change.levelup/{timestamp}
```

If the git extension is installed, use `git.feature` instead: `git.feature --name change.levelup/{timestamp} --base main`.
...
```
git.commit --message "change.levelup: {cdr_ref} - {title}"
```
...
cd "$TEAM_DIRECTIVES"
git add -A
git commit -m "change.levelup: {cdr_ref} - {title}"
```
...
```
git.publish --title "change.levelup: {cdr_ref} - {title}" --draft
```

If git extension is unavailable and MCP `create_pull_request` (GitHub) or `create_merge_request` (GitLab) is available:
1. Push the branch: `git push origin change.levelup/{timestamp}`
2. Create a PR with:
   - Title: `change.levelup: {cdr_ref} - {title}`
...
```bash
cd "{TEAM_DIRECTIVES}"
git push origin change.levelup/{timestamp}
# Then create a PR at: {repo_url}/pull/new/change.levelup/{timestamp}
```
...
║                                                              ║
║  Branch: change.levelup/{timestamp}                          ║
║  {✓ PR created: {pr_url}}                                    ║
